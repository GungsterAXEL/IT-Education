...
WITH RECURSIVE cte AS
(
SELECT 1 AS a, 1 * 1 AS square
UNION ALL
SELECT a + 1, (a + 1) * (a + 1) AS square
# SELECT a + 1, POW(a +1, 2) AS square -- можно заменить на функцию POW().
FROM cte
WHERE a < 10
)
SELECT * FROM cte;
...
в зависимости от сегмента и количество договоров всего по всем банкам.
*/
SELECT *,
MAX(OSZ) OVER(PARTITION BY TB) 'максимальная задолженность в каждом банке',
